...
catalog = f'{current_user_no_at}_demo_catalog' #cindy_test_catalog
...
cdr_stream_bronze = (spark.read.format("json")
                          .load("s3a://db-gtm-industry-solutions/data/CME/telco/CDR"))
(cdr_stream_bronze.write
   .format("delta")
   .mode('overwrite')
   .saveAsTable("cdr_stream_bronze")
)
...
(spark.read.format("json")
   ...
   .format("delta")
   .mode('overwrite')
   .saveAsTable("pcmd_stream_bronze")
)
...
df_towers.select(
                 ...
                 ).write.format('delta').saveAsTable('static_tower_data')

# COMMAND ----------

import pyspark.sql.functions as F
df_towers = spark.table("static_tower_data")
df_cdr_bronze = spark.table("cdr_stream_bronze")

(df_cdr_bronze.join(df_towers, df_cdr_bronze.towerId == df_towers.GlobalID)
              .write
              .saveAsTable("cdr_stream_silver"))

# COMMAND ----------

df_pcmd_bronze = spark.table("pcmd_stream_bronze")
(df_pcmd_bronze.join(df_towers, df_pcmd_bronze.towerId == df_towers.GlobalID)
              .write
              .saveAsTable("pcmd_stream_silver"))
...
df_cdr_silver = spark.table("cdr_stream_silver")
...
(df_cdr_pivot_on_status_grouped_tower
          .select("date",              
                  "towerId",           
                  "answered",          
                  "dropped",           
          ...
          .write
          .saveAsTable("cdr_stream_minute_gold"))
...
# df_pcmd_silver = spark.table("pcmd_stream_silver")
...
# (df_pcmd_pivot_on_status_grouped_tower.select("date",  
#                                             "towerId", 
...
#                                             "State")
#                                       .write
#                                       .saveAsTable("pcmd_stream_minute_gold"))
...
df_cdr_minute_gold = spark.table("cdr_stream_minute_gold")

(df_cdr_minute_gold.groupBy(F.window("date", "1 hour"), "towerId")
                  ...
                  .write
                  .saveAsTable("cdr_stream_hour_gold"))
...
import pyspark.sql.functions as F
df_cdr_minute_gold = spark.table("cdr_stream_minute_gold")
(df_cdr_minute_gold.groupBy(F.window("date", "1 day"), "towerId")
                    .agg(F.sum(F.col("dropped")).alias("dropped"),   
                        F.sum(F.col("answered")).alias("answered"), 
                        F.sum(F.col("missed")).alias("missed"),
                        F.sum(F.col("text")).alias("text"),
                        ...
                        F.first("window.start").alias("datetime"))
                    .write
                    .saveAsTable("cdr_stream_day_gold"))
