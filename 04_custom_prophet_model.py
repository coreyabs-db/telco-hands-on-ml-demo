...
towerIds_list = [
    ("{B8F4712B-F69B-40B2-BB38-0D6974424723}",),
...
df = (
    ...
    .groupBy("ds")
    .agg(F.sum("totalRecords_CDR").alias("y")))
display(df)
...
def fitForecastHourly(towerId_row):
    ...
    df = (
      ...
      .filter(
          ...
          (F.col("datetime") < '2023-05-15'))
      .select(
          F.col("datetime").alias("ds"),
          F.col("totalRecords_CDR").alias("y")))

    # df = sparkx.sqxl(
    #     "select datetime as ds, totalRecords_CDR as y from cdr_hour_gold where datetime >= '2023-05-01' and datetime < '2023-05-15' and towerId = '{}'".format(towerId))
    pandas_df = df.toPandas()
...
def forecastTowerHourlyResults(history_pd):
    ...
    with mlflow.start_run():
        ...
        # start the model logging
        mlflow.prophet.log_model(model, "modelHourly")
        mlflow.log_params(params)
...
results.write.mode("append").saveAsTable("cdr_hour_forecast")
...
def isAnomoly(df_current):
    ...
    df_forecast = spark.sql(
        "SELECT * FROM cdr_hour_forecast WHERE training_date = '{}' AND ds = '{}'".format(
            currentTrainingDatetime, currentTimeWindow))

    # anomoly logic
    df_current_plus_forecast = df_current.join(df_forecast, ["towerId"])
    df_current_plus_forecast_with_anomoly = (
        df_current_plus_forecast
        .withColumn(
            "anomoly",
            anomolyDetectUDF(
                F.col("totalRecords_CDR"),
                F.col("cdr_hour_forecast.yhat_lower"),
                F.col("cdr_hour_forecast.yhat_upper")))
        .withColumn(
            "anomoly_importance",
            anomolyImportanceUDF(
                F.col("totalRecords_CDR"),
                F.col("cdr_hour_forecast.yhat_lower"),
                F.col("cdr_hour_forecast.yhat_upper"))))
