...
print(current_user,xp_path)

# COMMAND ----------

#Let's get our auto ml experiment using the path defined in previous notebook. This is specific to the demo, it gets the experiment ID of the last Auto ML run and the best model.
client = MlflowClient()
#Get experiment ID
...
try:
  ...
  latest_model = client.get_model_version_by_alias(f"{catalog}.{schema}.{model_name}", "prod")
  ...
  client.set_registered_model_alias(name=f"{catalog}.{schema}.{model_name}", alias="prod", version=latest_model.version)
